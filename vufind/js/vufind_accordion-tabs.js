...
//scroll to orange bar if not showing 25% of panel
  ...
  if (window.innerWidth > 767) {
    $('.accordion-tabs-minimal.vufind').each(function(index) {
      $(this).children('li').first().children('a').addClass('is-active').next().addClass('is-open').show();
    });
  } 

  $('.accordion-tabs-minimal.vufind').on('click', 'li > a.tab-link', function(event) {
    if (!$(this).hasClass('is-active')) {
      event.preventDefault();



      var accordionTabs = $(this).closest('.accordion-tabs-minimal.vufind');
